...
## 5-2. 운영 모니터링 (2026-08-30 구축)

UptimeRobot 무료 플랜에 모니터 2종.

| 모니터 | 대상 | 잡는 상황 |
| --- | --- | --- |
| HTTP | `/api/health` | 서버 다운, SSL 만료, 도메인 문제 |
| Keyword | `/api/health`에서 `"reliabilityHealthy":true` 부재 시 알림 | 서버는 살아 있는데 수집·알림 실패율이 임계치를 넘은 상태 |

두 번째가 이 서비스의 핵심 실패 모드를 잡는다. 가격 추적은 사이트가 멀쩡해
보이는 채로 수집만 조용히 멈추는 일이 가장 흔하다.

홈이 아니라 `/api/health`를 감시한다. 홈은 DB가 죽어도 샘플 데이터로 200을
반환할 수 있어 진짜 장애를 가린다.
